...
- 写下一句今天的心情，并生成情绪花卡片。
- 记录区显示今日记录状态，帮助用户快速知道今天是否已经种下花。
- 心情输入框附近提供随机记录引导问题，可以换一个提示再开始写。
- 种花成功后提供“去花园看看”和“继续记录”的下一步入口。
- 记录区显示最近 1-3 条记录摘要，方便快速回看刚刚种下的花。
- 种花时可以记录 1-5 级心情强度和 1-3 个今日关键词。
- 根据情绪生成随机今日花语。
...
- 支持查看花卡片详情，在详情中编辑心情文字、强度、标签、收藏状态和补充日记。
- 支持按情绪筛选、关键词搜索、最新优先 / 最早优先排序。
- 支持只看收藏、按标签筛选、按心情强度筛选，并可组合多个筛选条件。
- 根据全部记录生成标签云，点击标签可以快速浏览相关花卡片。
- 显示当前筛选结果数量。
- 支持列表 / 日历视图切换，按月份查看每天的情绪记录。
- 日历视图支持上一月、下一月、回到今天，并可点击日期查看当天记录。
- 支持导出 `.txt` 情绪日记。
- 支持导出和导入 JSON 备份，导入时可选择合并或覆盖。
- 显示情绪统计、最近 7 天记录、情绪分布、最常出现情绪、连续记录天数和本周回顾。
- V2.4 增加最近 30 天记录趋势、心情强度分析、常用标签 Top 5、收藏记录回顾、情绪 × 强度和月度回顾摘要。
- V2.5 将分析区整理为洞察仪表盘，支持洞察总览和今日 / 本周 / 本月内部切换，移动端查看更清楚。
- V2.7 补强异常数据兜底、导入稳定性、搜索性能、移动端弹窗细节和 PWA 缓存版本。
- V2.8 在保持三文件结构的前提下整理代码分区、结构注释和文档说明，方便继续维护。
- V2.9 做最终产品体验验收，收口首次说明、记录路径、筛选状态、移动端遮挡、备份提示和文档一致性。
- 所有数据分析都基于全部记录，不受当前筛选、搜索、排序、标签、收藏或强度筛选影响。
...
- V2.2 增加的强度、标签、收藏和补充日记字段都是单条记录的可选字段，旧记录没有这些字段也可以正常显示。
- 标签、收藏、强度等筛选只影响当前页面显示，不会改变原始记录；日历、数据分析和导出仍基于全部记录。
- 首次使用引导和主题偏好使用单独的 `localStorage` key，不会改变花卡片数据结构。
- V2.7 增加了读取时的数据健康检查：异常字段会使用安全兜底，避免旧数据或异常数据导致页面报错。
...
- `V2.1`：增加日历视图，可按月份和日期查看情绪记录。
- `V2.2`：增加花卡片详情、心情强度、标签、收藏和补充日记。
- `V2.3`：增加收藏筛选、标签云、标签筛选和强度筛选。
- `V2.4`：增加最近 30 天趋势、心情强度分析、常用标签 Top 5、收藏回顾、情绪 × 强度和月度回顾摘要。
- `V2.5`：重组数据分析区，增加洞察总览和今日 / 本周 / 本月切换，优化移动端分析体验。
- `V2.6`：优化真实使用路径，增加今日状态卡、随机记录引导、种花后下一步提示和最近记录快速回看。
- `V2.7`：补强数据健康检查、旧记录兼容、导入稳定性、搜索 debounce、移动端弹窗和 PWA 缓存版本。
- `V2.8`：轻量整理 `script.js`、`style.css` 和 `index.html` 的分区结构，不新增业务功能。
- `V2.9`：最终产品体验验收，优化首次访问说明、每日记录路径、筛选状态提示、移动端遮挡细节和发布文档。
...
- 如何用原生 JavaScript 生成月历视图。
- 如何按本地日期把记录分组，避免 UTC 日期偏移。
- 如何在不改变原始数据结构的情况下增加新的浏览视图。
- 如何给单条记录增加可选字段，并兼容旧数据。
- 如何通过 `id` 打开详情弹窗并更新数组中的某一条记录。
- 如何解析用户输入的标签，并保存为数组。
- 如何让 txt / JSON 导入导出兼容新增字段。
- 如何根据全部记录统计标签云。
- 如何组合情绪、关键词、收藏、标签、强度和排序等多个筛选条件。
- 如何让筛选只影响当前显示，不改变原始数据。
- 如何基于全部记录计算最近 30 天趋势、强度分布、常用标签、收藏回顾和月度摘要。
- 如何把多个分析模块重新组织成总览、今日、本周和本月四个层级。
- 如何用内部 tab 只改变展示层，而不影响原始数据和筛选逻辑。
- 如何围绕“每天快速完成一次记录”的真实场景优化入口和反馈。
- 如何基于全部本地记录计算今日状态与最近记录，而不影响筛选后的显示列表。
- 如何为 `localStorage` 中的异常记录增加安全兜底，避免页面因为旧数据或损坏字段崩溃。
- 如何用轻量 debounce 降低搜索输入时的重复渲染。
- 如何检查移动端弹窗、Toast、底部导航和 PWA 缓存版本这些长期使用细节。
- 如何通过清晰分区注释整理长文件，让原生三文件项目更容易继续维护。
- 如何在不新增业务功能的情况下做最终产品体验验收和发布收口。
- 如何在旧记录缺少 `intensity`、`tags`、`isFavorite` 时保持数据分析稳定。
- 如何让数据回顾文案保持克制，只做记录总结，不做心理诊断。
